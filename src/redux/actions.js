...
export function* searchMoviesAsync(action) {
    const response = yield call(fetch, `https://reactjs-cdp.herokuapp.com/movies?search=${action.searchTerm}&sortBy=${action.sortBy}&searchBy=${action.searchBy}`); //TODO
    const movies = yield response.json();
...
export function* viewMovieByIdAsync(action) {
    ...
    const moviesByGenreResponse = yield call(fetch, `https://reactjs-cdp.herokuapp.com/movies?search=${movie.genres[0]}&sortBy=release_date&searchBy=title`); //TODO
